    ...
    while(a){
        ...
        {   //process available in ready queue 
            ...
            for (int x=0;x<pro[robin].no_res;x++){
               bool as = (pro[robin].max_resources+x)->lock_mutex((pro[robin].max_resources+x)->mtx);
               if(!as){
           ...
           if(a){
           if(pro[robin].interrupt_ar_time<pro[robin].burst_time-pro[robin].remaining_time+time_quant && pro[robin].type){//interrupt condition 
            ...
            else if (pro[robin].remaining_time<=time_quant & pro[robin].remaining_time>0){//process completion 
